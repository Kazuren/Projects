PlayerAirState = class('PlayerAirState', EntityBaseState)

function PlayerAirState:initialize(player, chapter)
    EntityBaseState.initialize(self, player)
    self.player = player
end

function PlayerAirState:enter(params)
...
function PlayerAirState:update(dt)
    ...
    self.player.dy = lume.approach(self.player.dy, self.player.dyMax, self.player.gravity)
    ...
    for i, collision in pairs(cols) do
        ...
        if collision.normal.y == -1 then -- bottom
            if collision.type ~= 'cross' then
                self.player.dy = 0
                if self.player.dx == 0 then
...
function PlayerAirState:__tostring()
    return 'Air'
end