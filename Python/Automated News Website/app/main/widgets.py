...
class CheckboxList(object):
  def __call__(self, field, **kwargs):
    ...
    for value, label, checked in field.iter_choices():
      ...
      html.append(u'<div class="custom-checkbox"><input %s class="custom-control-input"/> ' % html_params(**options))
...
class RadioList(object):
  def __call__(self, field, **kwargs):
    ...
    for value, label, checked in field.iter_choices():
      ...
      if checked:
        options['checked'] = 'checked'
      html.append(u'<div class="custom-radio"><input %s class="custom-control-input"/> ' % html_params(**options))
      html.append(u'<label for="%s-%s" class="custom-control-label">%s</label></div>' % (field_id, value, label))
    html.append(u'</div>')
    return  HTMLString(u''.join(html))
